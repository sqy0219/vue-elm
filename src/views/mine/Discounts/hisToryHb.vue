<template>
  <div class="hisToryHb">
    <div class="hea">
      <NavTop4 title="历史红包" icon6 />
    </div>

    <section class="hb">
      <ul>
        <li v-for="(hb,index) in hongbao" :key="index">
          <section class="list_item">
            <div class="list_item_left">
              <span class="sz">
                <span>¥</span>
                <p class="first">{{hb.amount}}</p>
              </span>

              <p>{{hb.description_map.sum_condition}}</p>
            </div>
            <div class="list_item_right">
              <h4>{{hb.name}}</h4>
              <p class="time">{{hb.description_map.validity_periods}}</p>
              <p class="phone">{{hb.description_map.phone}}</p>
            </div>
            <div class="time_left">
              <svg viewBox="0 0 126 126" id="expired">
                <path
                  fill="#9B9B9B"
                  fill-rule="evenodd"
          ...
          <footer class="list_item_footer" v-if="hb.limit_map">
            <p>{{hb.limit_map.restaurant_flavor_ids}}</p>
          </footer>
        </li>
      </ul>
    </section>
...
export default {
  ...
  methods: {
    getHongbao() {
      this.$axios
        .get("promotion/v2/users/1/expired_hongbaos?limit=20&offset=0")
        .then(res => {
          console.log(res);
...
.hb .list_item {
  ...
  margin: 0;
  padding: 0.46rem 0.24rem 0.38rem;
